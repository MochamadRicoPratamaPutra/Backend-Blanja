...
const getCategoryById = (id) => {
  return new Promise((resolve, reject) => {
    connection.query(`SELECT products.id, products.name, products.price, products.imgUrl, category.categoryID 
    FROM products INNER JOIN category ON products.categoryID = category.categoryID 
    WHERE category.categoryID = ?`, id, (error, result) => {
      if (!error) {
        resolve(result)
      } else {
        reject(error)
